...
def get_number_of_aliens_x(ai_settings, alien_width):
    """ get amount of aliens able to feet on screen)"""
...
def check_keyup_event(event, ai_settings, screen, ship, aliens, bullets):
    """ check key up event """
    if event.key == pygame.K_RIGHT: 
...
def check_bullet_alien_collision(ai_settings, screen, ship, aliens, bullets):
    ...
    if len(aliens) == 0:
        # Destroy all bullets and recreate board
        bullets.empty()
        create_fleet(ai_settings, screen, ship, aliens)
...
def fire_bullet(ai_settings, screen, ship, bullets):
    """ fire bullet if limit is no treached """
...
def check_aliens_bottom(ai_settings, stats, screen, ship, aliens, bullets):
    """ Check if any aliens have reached bot os screen """
